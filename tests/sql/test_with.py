    ...
    [
        [
            ...
            [
                ...
                # first query
                Select() \
                    .from_('cars', cols={'car_make': None, Literal('SUM(sales)'): 'total_sales_per_make'}) \
            ...
            [
                ...
                'sales_sum',
                # first query
                Select().from_('car_sales', cols={Literal('SUM(sales)'): 'total_sales'}),
                # columns
                [],
            ],
        ],
        # CTE query
        Select().from_("sales_per_make", cols=['car_make', 'total_sales_per_make']).from_("sales_sum",
                                                                                          cols=['total_sales']),
        # Expected result
...
with_recursive_cases = [
    ...
    [
        ...
        # with query
        Select().union([
            Select().from_({"tree": "t"}, cols=['id', 'link', 'data', Literal('0')]),
            Select().from_({"tree": "t"}, cols=['id', 'link', 'data', Literal('depth+1')]).join({"search_tree": "st"},
                                                                                                "link",
                                                                                                "t", "id")
        ], Sql.SQL_UNION_ALL),
        # columns
...
@pytest.mark.parametrize("name, with_query, columns, query, materialized, result", with_cases)
